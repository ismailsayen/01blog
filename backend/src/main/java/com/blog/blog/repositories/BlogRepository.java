...
public interface BlogRepository extends JpaRepository<BlogEntity, Long> {
    ...
    @Query(value = "SELECT  b.id AS id, b.categorie AS categorie, b.comments_count AS commentsCount, b.content AS content, b.created_at AS createdAt, b.hide AS hide, b.last_update_at AS lastUpdateAt, b.like_count AS likeCount, b.title AS title, b.user_id AS userId, u.user_name, u.job, u.avatar, EXISTS(SELECT 1 FROM reactions r WHERE r.user_id=:id AND b.id=r.blog_id) as liked, (b.user_id=:id) AS myBlog FROM blogs b INNER JOIN users u ON b.user_id=u.id WHERE b.user_id IN (SELECT following_id FROM follows WHERE  follower_id =:id) AND ((:lastId = 0) OR (b.id < :lastId)) AND b.hide=FALSE ORDER BY b.created_at DESC LIMIT 5", nativeQuery = true)
    List<BlogDTO.BlogOutput> findAllData(@Param("id") Long id, @Param("lastId") Long lastId);

    @Query(value = "SELECT  b.id AS id, b.categorie AS categorie, b.comments_count AS commentsCount, b.content AS content, b.created_at AS createdAt, b.hide AS hide, b.last_update_at AS lastUpdateAt, b.like_count AS likeCount, b.title AS title, b.user_id AS userId, u.user_name, u.job, u.avatar, EXISTS(SELECT 1 FROM reactions r WHERE r.user_id=:id AND b.id=r.blog_id) as liked , (:id=:idUser) AS myBlog FROM blogs b INNER JOIN users u ON b.user_id=u.id WHERE  b.user_id=:idUser AND b.hide=FALSE ORDER BY b.created_at DESC", nativeQuery = true)
    List<BlogOutput> findBlogsById(Long idUser, Long id);

    @Query(value = "SELECT  b.id AS id, b.categorie AS categorie, b.comments_count AS commentsCount, b.content AS content, b.created_at AS createdAt, b.hide AS hide, b.last_update_at AS lastUpdateAt, b.like_count AS likeCount, b.title AS title, b.user_id AS userId, u.user_name, u.job, u.avatar, EXISTS(SELECT 1 FROM reactions r WHERE r.user_id=:id AND b.id=r.blog_id) as liked , (b.user_id=:id) AS myBlog FROM blogs b INNER JOIN users u ON b.user_id=u.id WHERE  b.id=:idBlog AND b.hide=FALSE ORDER BY b.created_at DESC", nativeQuery = true)
